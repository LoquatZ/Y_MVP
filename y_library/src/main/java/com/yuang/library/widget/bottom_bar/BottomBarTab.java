package com.yuang.library.widget.bottom_bar;
...
public class BottomBarTab extends FrameLayout {
    ...
    private int mUnIconRes;
    private int mColorRes;
    private TextView mTvUnreadCount;

    public BottomBarTab(Context context, @DrawableRes int icon, @DrawableRes int unIcon, CharSequence title, @ColorRes int color) {
        this(context, null, icon, unIcon, title, color);
    }

    public BottomBarTab(Context context, AttributeSet attrs, @DrawableRes int icon, @DrawableRes int unIcon, CharSequence title, @ColorRes int color) {
        this(context, attrs, 0, icon, unIcon, title, color);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int defStyleAttr, @DrawableRes int icon, @DrawableRes int unIcon, CharSequence title, @ColorRes int color) {
        super(context, attrs, defStyleAttr);
        init(context, icon, unIcon, title, color);
    }

    private void init(Context context, int icon, int unIcon, CharSequence title, int color) {
        mContext = context;
        mIconRes = icon;
        mUnIconRes = unIcon;
        mColorRes = color;
...
//        mIcon.setColorFilter(ContextCompat.getColor(context, R.color.tab_un_select));
        ...
        mTvTitle.setTextColor(ContextCompat.getColor(context, R.color.tab_un_select));
        ...
        mTvUnreadCount.setBackgroundResource(R.drawable.tab_msg_bubble);
...
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.brown));//变幻底色
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, mColorRes));
        } else {
            mIcon.setImageResource(mUnIconRes);
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.tab_un_select));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.tab_un_select));
        }
    }
